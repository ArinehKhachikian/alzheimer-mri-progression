...
To understand what drives the performance, I ran a systematic ablation across four experimental conditions:
 
| Model | Features | AUC-ROC |
|-------|----------|---------|
| MedicalNet ResNet10 | MRI only (binary) | 0.578 |
| MedicalNet ResNet10 | MRI only (3-class) | 0.635 |
| MultimodalResNet | MRI + age/sex/education | 0.699 |
| Logistic Regression | MMSE + CDR + demographics | 1.000 |
| MultimodalResNet | MRI + all clinical features | 0.997 |
...
## Why MRI Still Matters Clinically
 
The ablation shows cognitive scores dominate in this well-characterized research cohort. In real clinical practice, structural MRI serves purposes cognitive scores cannot:
 
- **Exclusion diagnosis:** MRI rules out other causes of cognitive decline (tumors, strokes, normal pressure hydrocephalus) that produce similar MMSE/CDR profiles but require different treatment
- **Preclinical detection:** Hippocampal atrophy on MRI begins years before cognitive scores decline — future clinical AI targets the presymptomatic stage where MMSE is still normal
- **Scalability:** Cognitive assessments require trained clinicians; automated MRI analysis could enable population-level screening
- **Research cohort bias:** ADNI subjects have confirmed diagnoses with clearly separated cognitive scores — real deployment involves ambiguous early-stage presentations where imaging provides signal cognitive tests miss
---
 
## The Research Journey
 
**Attempt 1 — DenseNet121 from scratch (3-class)**
Heavy overfitting — train accuracy reached 97% while val accuracy plateaued at 47%. A 3-class problem with 325 training subjects is underconstrained for a 121-layer model trained from random initialization.
...
Adding five clinical features (MMSE, CDR, age, sex, education) as a parallel input stream produced a decisive improvement. The ablation then revealed MMSE and CDR are the primary drivers, motivating the MRI + demographics-only experiment (AUC 0.699) as the rigorous imaging contribution.
...
**Branches:**
- `main` — 3-class imaging-only pipeline (CN/MCI/Dementia), AUC 0.635
- `binary-classification` — binary CN vs Dementia, imaging only, AUC 0.578
- `multimodal-dataset` ← **you are here** — full multimodal fusion, AUC 0.997
- `multimodal-no-scores` — MRI + demographics only, rigorous ablation, AUC 0.699
---
...
- **Dataset size:** 330 binary subjects is small relative to published work. Larger cohorts would improve generalization.
- **No Grad-CAM for multimodal model:** Explainability for the fusion architecture is more complex than single-modality and was not implemented.
---
